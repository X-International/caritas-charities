...
export default function RelatedNewsCarousel({ currentArticleSlug, allArticles }: { currentArticleSlug: string, allArticles: NewsArticle[] }) {
    ...
    const next = () => setCurrentIndex(prev => (prev + 1) % relatedArticles.length);
    const prev = () => setCurrentIndex(prev => (prev - 1 + relatedArticles.length) % relatedArticles.length);
        ...
        <section className="space-y-4">
            ...
            <div className="relative group">
                ...
                {relatedArticles.length > 1 && (
                    <>
                        <button onClick={prev} className="absolute left-2 top-1/3 -translate-y-1/2 p-2 bg-white/80 rounded-full shadow-lg hover:bg-white transition-all duration-300 opacity-0 group-hover:opacity-100 z-10">
                            <ChevronLeft className="w-5 h-5 text-[#b10017]" />
                        </button>
                        <button onClick={next} className="absolute right-2 top-1/3 -translate-y-1/2 p-2 bg-white/80 rounded-full shadow-lg hover:bg-white transition-all duration-300 opacity-0 group-hover:opacity-100 z-10">
